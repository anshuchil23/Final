...
void draw() {
  ...
  if (b1.handleInput(mouseX, mouseY) && x>0) {
    x--;
  }
  if (b4.handleInput(mouseX, mouseY) && x<width) {
    x++;
  }
  if (b2.handleInput(mouseX, mouseY) && y>0) {
    y--;
  }
  if (b3.handleInput(mouseX, mouseY) && y<height) {
    y--;
  }
...
class Button {
  ...
  boolean handleInput(float xPos, float yPos) {
    // check to see if the mouse is inside the rect here,
    if(xPos<w && xPos>x && yPos>y && yPos<h){
      isPressed = true;
    }else{
      isPressed = false;
    }
    //if(dist(xPos, yPos, x, y)<w){
    // isPressed = true; 
    //}
